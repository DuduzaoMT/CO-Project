  ...
  if (_inFunctionBody) {
    ...
    if (node->initializer()) {
      ...
      if (node->is_typed(cdk::TYPE_INT) || node->is_typed(cdk::TYPE_STRING) || node->is_typed(cdk::TYPE_POINTER)) {
        ...
        // Assume initializer returns tensor pointer
        _pf.LOCAL(symbol->offset());
        _pf.STINT();
      } else {
    ...
    if (!_function) {
      if (node->initializer() == nullptr) {
        ...
        if (node->is_typed(cdk::TYPE_INT) || node->is_typed(cdk::TYPE_DOUBLE) || node->is_typed(cdk::TYPE_POINTER)) {
          ...
          // Global tensor initialization: call tensor_zeros and store pointer
          _pf.DATA();
          _pf.ALIGN();
          _pf.LABEL(id);
          auto dims = (std::dynamic_pointer_cast<cdk::tensor_type>(node->type()))->dims();
...
void udf::postfix_writer::do_add_node(cdk::add_node * const node, int lvl) {
  ...
  if ((node->left()->is_typed(cdk::TYPE_INT) || node->left()->is_typed(cdk::TYPE_DOUBLE)) && node->right()->is_typed(cdk::TYPE_TENSOR)) {
    node->left()->accept(this, lvl + 2);   
  ...
  if ((node->right()->is_typed(cdk::TYPE_INT) || node->right()->is_typed(cdk::TYPE_DOUBLE)) && node->left()->is_typed(cdk::TYPE_TENSOR)) {
      node->right()->accept(this, lvl + 2);  
...
void udf::postfix_writer::do_sub_node(cdk::sub_node * const node, int lvl) {
  ...
  if (node->right()->is_typed(cdk::TYPE_TENSOR) && node->left()->is_typed(cdk::TYPE_TENSOR)) {
    node->left()->accept(this, lvl + 2);  
    node->right()->accept(this, lvl + 2);  
...
void udf::postfix_writer::do_div_node(cdk::div_node * const node, int lvl) {
  ...
  if (node->right()->is_typed(cdk::TYPE_TENSOR) && (node->left()->is_typed(cdk::TYPE_INT) || node->left()->is_typed(cdk::TYPE_DOUBLE))) {
    node->left()->accept(this, lvl + 2);
    if (node->left()->is_typed(cdk::TYPE_INT)) {
      _pf.I2D();
    }
  ...
  if (node->left()->is_typed(cdk::TYPE_TENSOR) && (node->right()->is_typed(cdk::TYPE_INT) || node->right()->is_typed(cdk::TYPE_DOUBLE))) {
        node->right()->accept(this, lvl + 2);
    if (node->right()->is_typed(cdk::TYPE_INT)) {
      _pf.I2D();
    }
...
void udf::postfix_writer::do_ne_node(cdk::ne_node * const node, int lvl) {
  ...
  if (node->type()->name() == cdk::TYPE_TENSOR && node->left()->type()->name() == cdk::TYPE_TENSOR) {
...
void udf::postfix_writer::do_pointer_index_node(udf::pointer_index_node * const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
...
void udf::postfix_writer::do_return_node(udf::return_node *const node, int lvl) {
  ...
  if (_function->type()->name() != cdk::TYPE_VOID) {
    ...
    if (_function->type()->name() == cdk::TYPE_INT || _function->type()->name() == cdk::TYPE_STRING
      ...
      _pf.STFVAL32(); // FIXME: assuming tensor is represented as a pointer
    } else {
...
void udf::postfix_writer::do_tensor_node(udf::tensor_node *const node, int lvl) {
  ...
  auto flat_seq = node->expanded_fields();
  
  _functions_to_declare.insert("tensor_put");
  for (size_t i = 0; i < flat_seq->size(); i++) {
    _pf.INT(i);

    auto expr = dynamic_cast<cdk::expression_node*>(flat_seq->node(i));
    expr->accept(this, lvl + 2);

    if (expr->type()->name() == cdk::TYPE_INT) {
      _pf.I2D();
    } 
...
void udf::postfix_writer::do_tensor_index_node(udf::tensor_index_node * const node, int lvl) { 
  ASSERT_SAFE_EXPRESSIONS;
  node->positions()->accept(this, lvl + 2);
  node->base()->accept(this, lvl);
  _functions_to_declare.insert("tensor_getptr");
  _pf.CALL("tensor_getptr");
  _pf.TRASH((node->positions()->size()*4) + 4);
  _pf.LDFVAL32();
}
...
void udf::postfix_writer::do_tensor_reshape_node(udf::tensor_reshape_node *const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  for (int i = node->sizes()->size() - 1; i >= 0; --i) {
    auto expr = dynamic_cast<cdk::expression_node*>(node->sizes()->node(i));
    if (!expr) throw std::string("expected expression node in sizes sequence");
    expr->accept(this, lvl + 2);
  }
  size_t ndims = node->sizes()->size();
  _pf.INT(ndims);
  node->base()->accept(this, lvl + 2);
